...
class SokobanEnv(gym.Env):
    ...
    def __init__(self, level: Optional[np.ndarray] = None, max_steps: int = 200, render_mode: Optional[str] = None):
        ...
        if self.render_mode == "human":
            ...
            self.cell_size = 40
            self.screen_size = (
                self.original_level.shape[1] * self.cell_size,
                self.original_level.shape[0] * self.cell_size
            )
            self.screen = None
    ...
    def _render_pygame(self):
        ...
        # 색상 정의
        colors = {
            self.EMPTY: (255, 255, 255),          # 흰색
            self.WALL: (0, 0, 0),                 # 검은색
            self.BOX: (139, 69, 19),              # 갈색
            self.TARGET: (255, 0, 0),             # 빨간색
            self.PLAYER: (0, 0, 255),             # 파란색
            self.BOX_ON_TARGET: (0, 255, 0),      # 초록색
            self.PLAYER_ON_TARGET: (255, 0, 255)  # 자홍색
        }
        ...
        for i in range(self.level.shape[0]):
            for j in range(self.level.shape[1]):
                ...
                color = colors[self.level[i, j]]
                pygame.draw.rect(self.screen, color, rect)
                pygame.draw.rect(self.screen, (64, 64, 64), rect, 1)
    ...
    def _render_array(self) -> np.ndarray:
        ...
        color_map = {
            ...
            self.TARGET: [255, 0, 0],
            self.PLAYER: [0, 0, 255],
            self.BOX_ON_TARGET: [0, 255, 0],
            self.PLAYER_ON_TARGET: [255, 0, 255]
        }
        
        rgb_array = np.zeros((*self.level.shape, 3), dtype=np.uint8)
        for i in range(self.level.shape[0]):
            for j in range(self.level.shape[1]):
                rgb_array[i, j] = color_map[self.level[i, j]]
